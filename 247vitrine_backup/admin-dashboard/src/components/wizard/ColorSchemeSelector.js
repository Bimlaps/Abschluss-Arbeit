...
function ColorSchemeSelector({ onSelect, selectedColorScheme: initialSelectedColorScheme }) {
  const [colorSchemes, setColorSchemes] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedColorScheme, setSelectedColorScheme] = useState(initialSelectedColorScheme || null);

  useEffect(() => {
    // Wenn ein initialSelectedColorScheme übergeben wurde, setze es als ausgewähltes Farbschema
    if (initialSelectedColorScheme) {
      setSelectedColorScheme(initialSelectedColorScheme);
    }
  }, [initialSelectedColorScheme]);
