...
function DesignSelector({ layoutId, onSelect, selectedDesign: initialSelectedDesign }) {
  const [designs, setDesigns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedDesign, setSelectedDesign] = useState(initialSelectedDesign || null);

  useEffect(() => {
    // Wenn ein initialSelectedDesign übergeben wurde, setze es als ausgewähltes Design
  ...
  useEffect(() => {
    const fetchDesigns = async () => {
      try {
        ...
        // Zuerst das Layout abrufen, um die Kategorie zu erhalten
        const layoutResponse = await axios.get(`${API_URL}/api/layouts`);
        ...
        if (selectedLayout) {
          ...
          const designsResponse = await axios.get(`${API_URL}/api/designs/by-category/${selectedLayout.category}`);
          setDesigns(designsResponse.data);
        } else {
          // Fallback: Designs nach Layout-ID abrufen
          const designsResponse = await axios.get(`${API_URL}/api/designs/by-layout/${layoutId}`);
          setDesigns(designsResponse.data);
        }
